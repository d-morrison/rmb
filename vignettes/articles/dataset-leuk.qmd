...
```
Directed acyclic graph for the leukemia remission treatment comparison.
:::
...
dat$group_plot <- factor(dat$group, levels = c(1, 2), labels = c("6-MP", "Placebo"))
...
```
Kaplan-Meier remission curves comparing 6-MP and placebo groups.
:::
...
```
Swimmer plot of remission duration with relapse markers for all patients.
:::
...
```
Log-rank test results comparing remission curves across treatment groups.
:::
